...
;(function() {
	...
	emoji.prototype.replacement = function(idx, actual, wrapper, variation, is_extra){
		...
		if (self.supports_css) {
			if (self.use_sheet && img.px != null && img.py != null){
				var img_padding = 1;
				var padded_img_size = img.sheet_size + 2 * img_padding;
				var sheet_size = self.sheet_size * padded_img_size; // size of image in pixels
				var sheet_x = 100 * ( (img.px * padded_img_size) / (sheet_size - padded_img_size) );
				var sheet_y = 100 * ( (img.py * padded_img_size) / (sheet_size - padded_img_size) );
				var sheet_sz = 100 * (sheet_size / padded_img_size);
	...
	emoji.prototype.find_image = function(idx, var_idx){
		...
		for (var j=0; j<try_order.length; j++){
			if (use_mask && self.img_sets[try_order[j]] && self.img_sets[try_order[j]].mask){
